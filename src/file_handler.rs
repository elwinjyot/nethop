...
pub fn read_queries_from_file(path: &str) -> Result<String, String> {
    ...
    if !path.exists() {
        return Err(format!("{} no such file or directory", path.to_string_lossy()));
    }

    let queries = fs::read_to_string(path).map_err(|e| format!("Could not read the passed file, {}", e))?;
    Ok(queries)
}
...
pub fn read_queries_from_workspace() -> Result<String, String> {
    ...
    let headers = fs::read_to_string(&config_path)
        .map_err(|_| "Failed to read base config.hop file")?;
    ...
    for entry in WalkDir::new(&path).into_iter().filter_map(|et| et.ok()) {
        ...
        if file_path.is_file() 
           && file_path.extension().map_or(false, |f| f == "hop") 
           && file_path != config_path 
        {
            ...
            files_found += 1;
            println!("🕸️  {} file added", file_path.file_name().unwrap_or(OsStr::new("")).to_string_lossy());
        }
    }
